  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-ink">Eventos de integração</h1>
      ...
      {error && <p className="mt-4 text-sm text-red-600">Erro ao carregar eventos: {error.message}</p>}
            ...
            <tbody>
              {eventos.map((evento) => (
                <tr key={evento.id} className="border-b border-zinc-100 last:border-0">
                  <td className="px-4 py-3 text-muted">{new Date(evento.created_at).toLocaleString("pt-BR")}</td>
                  <td className="px-4 py-3 font-medium text-ink">{evento.provedor}</td>
                  <td className="px-4 py-3 text-ink">{evento.evento}</td>
                  <td className="px-4 py-3">
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                        evento.sucesso ? "bg-brand-green/10 text-brand-green-dark" : "bg-red-100 text-red-800"
                      }`}
                    >
                      {evento.sucesso ? "Sucesso" : "Falha"}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-xs text-red-700">{evento.mensagem_erro ?? "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
